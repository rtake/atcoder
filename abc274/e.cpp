...
double f(ll x0, ll y0, ll x1, ll y1) {
...
int main() {
  ll n,m; cin>>n>>m;
  vector<ll> x(n),y(n),p(m),q(m);
  rep(i,n) cin>>x[i]>>y[i];
  rep(i,m) cin>>p[i]>>q[i];

  vector< vector <double> > dp(1LL<<n, vector<double>(n,1e18));
  vector< vector< set<ll> > > dps(1LL<<n, vector< set<ll> >(n));
...
///*
        if((j>>k)&1LL == 0LL) {
          dp[j|(1LL<<k)][k]=min(dp[j|(1LL<<k)][k],dp[j][]);
        }
//*/
