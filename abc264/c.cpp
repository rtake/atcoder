...
int main() {
  ...
  for(ll bit0=0;bit0<(1LL<<h1);bit0++) {

    set<ll> st0;

    rep(i,h1) {
      if((bit0>>i)&1LL == 1LL) st0.insert(i);
    }

    for(ll bit1=0;bit1<(1LL<<w1);bit1++) {

      set<ll> st1;

      rep(i,w1) {
        if((bit1>>i)&1LL == 1LL) st1.insert(i);
      }
      
      if(!(st0.size() == h2) || !(st1.size() == w2)) continue;
