...
int main() {
  ...
  while(!dq.empty()) {
    ...
    if(r+1<h && s[r+1][c] == '.' && a[r+1][c] == -1) {
      a[r+1][c]=a[r][c]+1;
      dq.push_back((r+1)*w+c);
    }

    if(r-1>=0 && s[r-1][c] == '.' && a[r-1][c] == -1) {
      a[r-1][c]=a[r][c]+1;
      dq.push_back((r-1)*w+c);
    }
  
    if(c+1<w && s[r][c+1] == '.' && a[r][c+1] == -1) {
      a[r][c+1]=a[r][c]+1;
      dq.push_back(r*w+c+1);
    }
      
    if(c-1>=0 && s[r][c-1] == '.' && a[r][c-1] == -1) {
      a[r][c-1]=a[r][c]+1;
      dq.push_back(r*w+c-1);
    }
  ...
  // cout<<bcnt<<" "<<a[h-1][w-1]<<endl;
  cout<<h*w-bcnt-a[h-1][w-1]<<endl;
