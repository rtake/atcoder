...
ll f(ll cur, ll par) {
  ...
  for(auto p:V[cur]) {
    ...
    wmax[cur]=max(wmax[cur],w);
    wmax[cur]=max(wmax[cur],wmax[u]);
...
int main() {
  ...
  rep(i,n-1) {
    cin>>u>>v>>w;
    u--,v--;
    V[u].emplace_back(w,v);
    V[v].emplace_back(w,u);
  }

  ans[0]=f(0,-1);

  rep(i,n) cout<<cnt[i]<<" "; cout<<endl;
  rep(i,n) cout<<wmax[i]<<" "; cout<<endl;
