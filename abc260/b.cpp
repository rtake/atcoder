...
int main() {
  ...
  sort(ALL(bb),greater< pair<ll,ll> >());
  sort(ALL(cc),greater< pair<ll,ll> >());
  ...
  ll i=0;
  while(y > 0) {
    if(st.find(-bb[i].second) == st.end()) {
      st.insert(-bb[i].second);
      y--;
    }
  ...
  ll j=0;
  while(z > 0) {
    if(st.find(-cc[j].second) == st.end()) {
      st.insert(-cc[j].second);
      z--;
    }
