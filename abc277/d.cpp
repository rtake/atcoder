...
int main() {
  ...
  rep(i,n) {
    ...
    if(mp.find((a[i]+1)%m) != mp.end()) {
      uf.unite(i,mp[(a[i]+1)%m]);
      ok=true;
    }
    
    if(mp.find((a[i]-1+m)%m) != mp.end()) {
      uf.unite(i,mp[(a[i]-1+m)%m]);
      ok=true;
    }
/*
*/
...
/*
  rep(i,n) cout<<uf.find(i)<<" "; cout<<endl;
