...
int main() {
  ...
  rep(i,n) {
    ...
    while(!dq.empty()) {
      ...
      for(auto nxt:G[top]) {
        edge_set.insert({min(nxt,top), max(nxt,top)});
    ...
    if(ok) {
      ...
      nvec.push_back(cnt*(st.size()-cnt) - edge_set.size());
